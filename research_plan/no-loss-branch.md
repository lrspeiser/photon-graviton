...
## 6. Internal-state transport audit — 10 September 2026

The [internal companion reservoir calculation](../research_work/results/internal-companion-reservoir/report.md) constructs a conditional escape from the separate no-internal-state shared-speed restriction. With H=p/(1+a*s)+R(P), a constant-slope R makes the internal rate s_dot=1 while R absorbs the propagation-term loss. Total packet energy and reference light speed are both retained. A conventional quadratic R conserves energy but changes that rate and speed. The exact construction uses known extended-coordinate Hamiltonian mathematics and an explicitly hypothetical receiving state; it has no derived photon-conversion, creation/synchronization or capture mechanism. Its pre-existing reservoir energy is recorded separately and cannot be credited to photon loss. This advances the transport specification without passing the complete no-loss branch or reopening the deferred total universe source budget.
