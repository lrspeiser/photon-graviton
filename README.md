...
- **Latest: [timing estimator injections](research_work/results/timing-injection/report.md).** A frozen test on DES cadence patterns found duration/shape-dependent rejection in the first observer-time width estimator. The feasibility gate failed and no real-flux timing exponent was fitted. All trials remain available; partial coverage and shape variation must be handled before observational inference.
