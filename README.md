...
- **Latest: [spherical propagation](research_work/results/spherical-propagation/report.md).** In eight coupled spherical backgrounds, a steady compact source approaches a static field: new signals retain delay but their additional redshift becomes much smaller. Seventy-four carrier/event comparisons pass, with finite emitter fuel accounted. This limits the localized free-scalar candidate, not all cumulative-time mechanisms. [Companion self-binding and the time formula](research_plan/time-field-and-self-binding.md) are recorded as the next research questions.
