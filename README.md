...
- **Latest: [timing failure diagnosis](research_work/results/timing-diagnosis/report.md).** Errors in estimated event widths can dominate rejection bias, and apparently correct timing slopes can result from cancelling errors. The next estimator must handle uncertain shapes and partial coverage before real-flux inference.
