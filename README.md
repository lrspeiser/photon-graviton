...
**Current status:** [CURRENT-STATUS.md](CURRENT-STATUS.md) is the single authoritative summary: reviewed commit, active candidates, frozen benchmarks, unresolved failures and the next experiment. Dated notices that used to accumulate on this page are preserved in [CHANGELOG.md](CHANGELOG.md).
...
0. **[Current status](CURRENT-STATUS.md).** Read this first; it supersedes older "latest" notices.
...
## Earlier core findings

These summaries date from 9 September 2026 and are retained for provenance. See [CURRENT-STATUS.md](CURRENT-STATUS.md) for the present state of each candidate.
...
The current command checks original source hashes, runs diagnostics, and verifies that original files and saved results remain unchanged. Each invocation creates a fresh directory under `research_work/generated/`, with logs and a verification summary. `--output-dir` selects another new or empty directory. The default suite now has 49 jobs: the 45 recorded in the [pair-production-balance verification](research_work/pair-production-balance-verification.json), plus regeneration of the three latest canonical diagnostics (MOND-guided SPARC and Milky Way, Coma inverse), each compared numerically with its archived result, and the [capture-to-orbit verification checks](research_work/results/capture-to-orbit/checks.py). The optional historical baseline adds one more job. The full capture-to-orbit comparison takes several minutes on eight cores and is run separately: `python research_work/results/capture-to-orbit/runner.py`. Earlier verification records retain their original suite scope and results.
