...
- **Latest: [multiple regions in a shared companion field](research_work/results/multiple-regions/report.md).** Redshift accumulates without resetting the field. Thirty-two independent carrier/event comparisons match, but stretch changes with radiation loading and launch time. A frozen per-region multiplier and a single affine stretch for an entire event are not justified by this finite-burst example. The source population, matter clocks, capture and gravity remain incomplete.
