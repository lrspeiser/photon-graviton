...
- [All-direction capture](research_work/results/isotropic-capture/derivation.md) shows why an isotropic incoming bath is not automatically momentum-balanced for a moving receiver. Constant absorption produces slowing; a specified spectral response cancels it in an ideal comparison. Deriving that response, reverse processes and realistic evolution remains necessary.

- [Capture momentum](research_work/results/capture-momentum/derivation.md) connects storage to the push and orbital changes experienced by its receiver. It quantifies directional-force limits under declared loading conditions; tiny packets do not remove the mean momentum delivered per unit energy. Actual capture geometry and self-consistent evolution remain unresolved.

- [Deposit support](research_work/results/deposit-support/derivation.md) separates internal storage lifetime from residence at a galactic radius. The calculations quantify infall without support, orbital requirements and an ideal positive equilibrium, while showing why an isotropic radiation store needs a different pressure treatment. Formation and a finite self-consistent reservoir remain unresolved.
