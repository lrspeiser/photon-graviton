...
Working paper v1.5 | 13 September 2026 | Evidence through revision 2929cc8
...
The subsequent receiver-recoil calculation keeps capture open kinematically. Under a chosen recoil budget equal to each profile's orbital K/(M_d c squared), a participating receiver rest energy about 7170-677000 times the incident packet energy permits separate-particle production at rest with unchanged receiver invariant mass. These ratios are conditional budget thresholds, not minimum masses required for any capture. Absorption into the receiver's internal state is a distinct branch. The calculations apply established local energy-momentum kinematics [25], not a new interaction law. They do not determine production rates or an accessible particle mass spectrum. A particle created at rest also lacks the angular momentum of the circular support construction. Recoil need not become heat immediately, and ordinary-matter absorption does not automatically create an extended independent reservoir. This narrows the next question from whether momentum can balance to how a local interaction populates the required bound orbits.
...
The following outcomes retain the full work program. They are not an execution order or claims already achieved. Section 8 selects the next inexpensive discriminating calculations from this broader scope. Each requirement remains open until its own evidence is supplied.
...
### 8. Prioritized next steps: reuse existing calculations first

The immediate objective is to resolve the largest ambiguities with small, auditable experiments using the files already available. No new catalog hunt, full cosmological simulation or unrestricted parameter sweep is needed for the first three tasks. All existing samples are exposed: they can diagnose failures and guide development, but cannot become new blind evidence. The order below reflects expected information gained per calculation, not confidence that a branch will succeed. These tasks are proposed, not executed in this revision.

#### 8.1 First: compare the existing propagation fits consistently

Question: does one constant conversion rate explain both nearby redshifts and supernova brightness under one explicitly chosen duration/beam rule? The nearby value 74.62 and conditional brightness value 70.48 already suggest a tension, and changing the constant alone cannot remove the recorded redshift-dependent brightness trend. Another isolated redshift fit would add little.

Smallest useful calculation: reuse direct-conversion/results.json, brightness-distance-consistency/rate-results.json, the archived supernova covariance and calibrators. Evaluate the existing duration-stretched static-beam branch at the nearby rate, at its brightness optimum and at a jointly selected rate. Complete the missing flat-FLRW brightness comparison with the same supernova rows, covariance, calibrator treatment and nuisance freedom, fitting H0 with the specified 0.3/0.7 background. Distinguish the models' physical distance interpretations; holding a catalogue value fixed does not establish model-independent calibration. Audit shared calibration before adding likelihoods; if its cross-covariance is unavailable, report separate conditional scores and a sensitivity range rather than a precision joint confidence interval.

Deliverable: one compact table of nearby-redshift residuals, brightness residuals by redshift bin, duration consistency and fitted-parameter counts for each candidate. Retain the existing 1+z duration assumption as an assumption, not a derived success. Why first: the calculations and data already exist, and this exposes whether a single rate works or a shape/beam change is necessary. Stop condition: if the trend persists, do not keep tuning alpha; identify which propagation assumption requires revision. A better score alone does not derive the photon-companion mechanism.

#### 8.2 Second: test whether capture leads to usable orbits

Question: can a conserving local capture prescription populate a long-lived extended reservoir rather than merely stop the incoming energy? The latest recoil calculation removes a kinematic objection, but stopping a particle and putting it on a circular orbit are different achievements.

Smallest useful calculation: reuse the refined ten-profile potentials and angular radiation field. Compare at most three declared injection prescriptions: particles initially at rest; a fixed isotropic slow-velocity distribution; and a tangentially biased distribution with one shared width. These are illustrative proposed prescriptions, not derived microscopic interactions. For each, calculate local binding energy and angular momentum, turning radii and the receiver/outgoing momentum required by conservation. Compare orbit residence with the prescribed density. Use a fixed potential as a first necessary test; do not label it collective stability or self-consistent formation. A circularity distribution that was imposed cannot count as an explanation of its own origin.

Deliverable: bound fraction, radial/tangential motion, recoil budget and predicted redistribution by radius, including which results depend on the chosen receiver mass or injection energy. Why now: capture is kinematically possible, and these inexpensive orbit calculations can expose whether the simplest versions destroy the hollow profiles that helped lensing. Stop condition: retain a branch only as a candidate if it preserves the relevant profile and balances energy/momentum; otherwise name the missing torque, cooling or support process. Do not manufacture a successful profile by assigning a separate injection law to each galaxy.

#### 8.3 Third: locate the shared-law error before adding a parameter

Question: which systems and radii account for the remaining rotation and lensing failures? The shared exact-third law improves ordinary matter but trails the tested MOND rule. Free companion normalization improves lens fits while ceasing to test that law. These are not interchangeable successes.

Smallest useful calculation: reuse saved per-radius SPARC predictions and the six lens residual/covariance records. Without fitting, separate residuals by radius, surface brightness and gas fraction using bins declared before this diagnostic. Report signed errors and uncertainty, not only pooled RMS. Keep the galaxy-rotation objective and lens likelihood distinct, and compare each with its existing ordinary-matter, MOND or NFW controls where actually executed. Identify whether the main failure is normalization, radial shape or a small number of objects. In particular, preserve the fact that only two of six lens systems improve under the transferred profile.

Deliverable: a short residual map and one explicitly motivated candidate modification, or a finding that no shared modification is justified. Why this is inexpensive: the predictions already exist and no new physical parameters are needed for the diagnostic. Stop condition: test at most one additional shared parameter on the existing training subset, retain all comparison outcomes, and reject promotion if improvement comes only from target-specific freedom or damages previously successful limits. Previously inspected validation/test partitions remain development checks. Freeze the resulting rule and protocol before a genuinely untouched-system test.

#### 8.4 Work unlocked by those results

If capture survives the orbit check, calculate a local interaction rate, causal receiver size and cumulative momentum diffusion, then connect radiation supply and retention to the density amplitude. Keep source history free but quantify what history is required; do not use unlimited age to hide a deficit. If the shared gravity residual has a defensible correction, complete the extended-baryon Coma and matched six-galaxy MOND comparisons, recording the chosen lensing completion and matter uncertainties. These are important but require more modeling than the immediate reused-data checks.

Collective stability, a derivation of one-third retention, full Milky Way distribution modeling, emission spectra and the broader microwave/structure/abundance program remain requirements in Section 6. Companion-powered black-hole emission remains optional; recoil accounting alone predicts neither unavoidable heat nor Hawking-like radiation. Pursue these after the cheaper tests clarify the required mechanism. This ordering changes priority, not the ultimate scope.

Every experiment must state known equations, new assumptions, input provenance, fixed and fitted parameters, numerical checks, failed outcomes and the consequence for the next choice. Keep incompatible propagation and storage branches separate. A useful failed calculation is one that tells us what to change; repeated refinement without changing the physical assessment is not the priority. No background execution is implied by this agenda.

### 9. Conclusion

The project has established a useful empirical gravity family and a calibrated redshift curve, with numerical checks and explicit failures. Shared companions improve ordinary-matter rotation but do not beat the tested MOND rule. Freely fitted companion profiles improve on matched NFW fits in six lens galaxies, at the cost of removing the shared normalization test; Coma does not distinguish the main fitted shapes. The extreme reservoir is not required, and idealized tangential orbital support is possible, but capture, collective stability and absolute supply remain open. Nearby conversion and expansion curves have almost identical errors, while brightness favors a different conditional conversion rate. A single mechanism has not yet connected these successes to event timing and the remaining observations. The immediate priorities are a consistent existing-data propagation comparison, a conserving capture-to-orbit diagnostic, and a residual map that identifies whether a small shared-law change is justified. Full supply, formation and untouched-system validation remain necessary afterward. These determine whether the model predicts new outcomes rather than assigning a separate rule to each phenomenon. No replacement for MOND, dark matter or expanding cosmology is established.
