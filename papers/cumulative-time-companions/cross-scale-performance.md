...
- **A written track is a different proposal, and it survives further — but not far enough yet.** A body that leaves a fading footprint along its own path does build an attractive, confining groove, and a second body feels it. The cost is that the body is always climbing out of the hole it has just dug: the support it gains grows with how long the groove lasts, while that drag does not. Surviving long enough to finish the groove then demands a groove almost as wide as the orbit, at which point it is no longer a path.
