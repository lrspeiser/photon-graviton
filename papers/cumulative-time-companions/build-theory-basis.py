...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path(__file__).parents[2] / "output/pdf/theory-basis.pdf")
    args = parser.parse_args()
    text = Path(__file__).with_name("theory-basis.md").read_text(encoding="utf-8")
    ...
    for index, block in enumerate(blocks):
        if block.startswith("$$"):
            equation_count += 1
            equation = Equation(block.strip().removeprefix("$$").removesuffix("$$").strip())
            equation.keepWithNext = index + 1 < len(blocks) and blocks[index + 1].startswith("$$")
    ...
    doc = SimpleDocTemplate(str(args.output), pagesize=(612, 792), rightMargin=58,
                            ...
                            author="", subject="Working theoretical framework, version 1.4")
    ...
    def page(canvas, document):
        ...
        canvas.drawString(58, 28, "Working draft v1.4  |  13 September 2026  |  Theory incomplete")
