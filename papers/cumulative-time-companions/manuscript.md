...
The subsequent autonomous homogeneous matter-clock calculation makes a crucial limitation explicit. When q=1/n multiplies both the photon Hamiltonian and local matter clocks, the measured stretch is (n_o/n_e)(q_o/q_e)=1. The same fixed-distance ray solution gives equal emitted and received intervals in local clock time. This is established clock and ray bookkeeping within the assumed model, not a new formula. It does not contradict the prescribed spatially varying examples with matched endpoint clocks above, but prevents those examples from being advertised as a completed universal-clock mechanism. Ordinary matter also exchanges energy with the receiver in this completion; that energy cannot all be attributed to photons [5, matter-clock-closure].
...
#### 8.8 Three-dimensional gravity and reserved stellar validation

The local observational inventory now includes APOGEE DR17 (733,901 allStarLite rows), StarHorse (562,424 rows), Gaia covariance information for 140,407 matched rows, and 8,585 BRAVA rows. The prepared matched catalog contains 128,772 quality-selected rows. These counts establish available inputs, not independent measurements of the gravitational field. Survey selection, distance priors, stellar populations and nonaxisymmetric bulge orbits still require a joint likelihood. In particular, stars beneath the bulge and above or below it cannot be compared by assigning each star a unique predicted circular speed [5, stellar-orbit-support].

The tested three-dimensional response places the archived empirical acceleration relation inside a known conservative, QUMOND-type Poisson field construction. This ensures that radial and vertical forces come from one potential; neither that mathematical structure nor the Jeans equations used for inference are claimed as new. Identifying the additional field with deposited photon energy remains a separate hypothesis. The ordinary-matter baseline includes stellar disks, gas, central stellar components and the central black hole, without an active dark halo.

These exposed summaries are conditional, model-dependent benchmarks: the published vertical analysis includes halo-model assumptions, which are not imported as an active component here. They cannot substitute for a new raw-star inference. On these benchmarks, the added field reduces the radial-speed RMS from 67.44 to 8.61 km/s, but increases the vertical-force RMS from 27.99 to 33.31 in the archived surface-density-equivalent units. Changing the depth or shape of a potential changes its gradients: the deposited energy need not remember an arrival direction for the resulting gravity to have both radial and vertical components. Thus fitting rotation alone does not determine the bulge or disk-height response. Potential-shape and ordinary-component experiments expose this tradeoff; they do not yet identify a unique deposition geometry [5, conservative-field-completion; potential-shape-response; baryon-component-response].

A later evaluation froze its code, field coefficients, calibration, selection and two candidate models in commit e5cf484 before computing the reserved-star outcomes. Of 433 measurement/mode candidates, 167 stars pass the fixed cuts. Nine one-kpc bins from 6 to 15 kpc meet the declared minimum count; four selected stars in the three outer bins are retained in the record but do not supply scored bins. The inferred circular speeds use population moments and a simplified Jeans model, rather than direct acceleration measurements.

| Frozen model | Validation RMS (km/s) | Mean residual (km/s) | Bins below inferred speed |
|---|---:|---:|---:|
| Original ordinary matter | 67.74 | -66.83 | 9/9 |
| Original added field | 15.36 | -13.83 | 9/9 |
| Adjusted ordinary matter | 64.52 | -63.28 | 9/9 |
| Added field with adjusted matter | 12.36 | -9.91 | 8/9 |

The secondary model fixes stellar-disk, gas-disk and central-stellar multipliers at 0.961997, 0.700000 and 1.300000, respectively. These were chosen using training/exposed data within an illustrative sensitivity range, not an independently measured mass confidence interval. No new parameter or potential-shape factor was fitted to validation. Coarse/refined differences remain below 0.0099 km/s. The secondary reduces validation RMS by 19.6 percent relative to the primary, but substantial systematic underprediction remains.

This is a held-out star subset sharing Gaia calibration and Galactic populations with training; published aggregate Cepheid curves had already been examined. It is not an independent catalog or a complete uncertainty and selection analysis. These validation outcomes are now exposed. The final Cepheid test role remains unopened, and the inner bulge is outside this radial selection. The result cannot be used to claim a completed bulge comparison or validation of photon origin [5, cepheid-validation].

#### 8.9 Mechanism and transfer tests after the initial draft

A known scalar-dependent electromagnetic kinetic coupling permits energy exchange when opposed waves overlap, while an isolated ideal null traveling wave does not source the scalar through the same invariant. Carrier-resolved tests show why energy exchange alone is insufficient: one resonant example transfers approximately 3.611 percent of initial energy to the scalar but shifts the outgoing energy-weighted spectral centroid upward by approximately 0.221 percent. It does not uniformly redshift the spectrum. These are dimensionless numerical experiments, not a measured astronomical conversion rate or evidence of permanent capture [5, gauge-kinetic-transfer; gauge-kinetic-spectrum].

Production and storage also require compatible states. Under established vacuum two-photon-to-one-scalar kinematics, the tested ultralight scalar can be produced with enormous kinetic energy relative to its rest energy. This is not automatically the slow bound configuration used in a galaxy equilibrium calculation. Momentum transfer, redistribution and retention must be supplied by a specified interaction. This state-matching issue is distinct from the total photon-supply budget, which remains deferred rather than passed [5, companion-source-matching].

The finite wave-equilibrium lens candidate improves stellar velocity-dispersion RMS relative to a fitted ordinary-mass control on the exposed 12-object SLACS test, but worsens Einstein-radius RMS. Its small combined-score advantage is sensitive to one object. Transferring its fixed source-to-stellar mass ratio of 0.6 to disk galaxies fails a generous necessary outer-force bound in 147 of 149 exposed SPARC galaxies. This rules out that frozen normalization as a universal solution under the tested assumptions, not all companion-source models. No lens equilibrium calculation yet derives its source from the photon-conversion mechanism [5, wave-lens-test; wave-disk-transfer-bound].

The intended transport branch still requires no companion energy loss before capture and durable storage after capture. These are explicit requirements, not consequences demonstrated by the exchange equations. A complete model must account for momentum, binding, reverse reactions and the energy of every receiver or driver. Successful energy conservation in a toy Hamiltonian does not by itself establish redshift, transient stretching or a stable gravitational reservoir.
...
The central proposal can be made precise enough to test, but its components have not been joined into a single successful mechanism. Prescribed evolving propagation fields can stretch a whole signal; separate interactions can exchange energy. The tested homogeneous universal-clock completion instead cancels observable stretching, and the tested resonant conversion does not produce the required spectral shift. The present work supplies conditional realizations of parts of that mechanism. It does not yet explain from fundamental principles why low-gravity regions establish the required field, why local instruments remain compatible, how companions become a stable extended source, or whether their predicted gravity is sufficient.
...
[5] Photon-Companion Research repository (2026). Conditional derivations, diagnostics and research reports. [Evidence snapshot](https://github.com/lrspeiser/photon-graviton/tree/4f83fe5). Relevant report identifiers are supplied in the accompanying evidence map. Repository results are internal working evidence, not peer-reviewed validation.
