...
| **R25** Flow frustration tested as registered (round 25) | **Not supported: the misses do not grow with the share of the companion's energy that is not flowing (pooled p = 0.44; passes median chi 0.092, misses 0.006; all three registered conditions fail), because the law already spends the hot glow's non-flowing energy in the pull's size; a companion that keeps turning (τ = u/a, 14.8 kpc) is excluded by lensing and flat rotation curves. Leads found after looking: the misses are one-sided (36 of 43 low), 8–16% even with nothing frustrated, and the dwarfs are ordered by the Galaxy's pull over their own (6 of 6 with η ≥ 0.78 miss, 4 of 4 with η ≤ 0.31 pass), the case `no_hold_r12` addresses** | **Registration:** `round25-flow-frustration.md` (commit e74f3f8), before any value. **Test** (`code/frustration_v25.py`, 3,435 points): SPARC ρ −0.03; X-COP +0.28, −0.56 at fixed radius; Milky Way +0.09; dwarfs −0.37; collisions +0.10 (all p > 0.01); KiDS and Mistele chi = 0 exactly (point lenses) with the law 16% and 13% low. **Exact:** Q = Q_hot + Q_dir; flicker m² + f² = 1 for two opposed coherent streams. **Turning** (`code/frustration_transport_v25.py`): reach 211 kpc in 13 Gyr (straight: 2,253), energy ×9 at 60–150 kpc (random walk agrees). **Exploratory** (`code/frustration_explore_v25.py`): sign test p = 9 × 10⁻⁶; dwarfs ρ(η) = +0.87. DRT-1 (main): separate brightness alone keeps 9.2% of a dwarf's isolated response at 100:1, so the force rule itself must change. **DRT-1 against the data** (README §36.7): the joining rule's unbounded region covers X-COP, most of SPARC, the Milky Way inside 27 kpc and the Bullet; cluster gas may keep its own glow only to τ ≲ 2; retention where streams oppose flattens X-COP's radial trend (0.239 → 0.108) but, through a refitted u, costs galaxy lensing (KiDS all 0.065 → 0.12 dex); no piece closes the Bullet's smaller half (+14–21% of the +43% needed). **The owner's self-energy repair** (README §36.8): the bound checks; its own packet dynamics traps the hot glow in galaxy interiors (n < 0 where Q > a/4 + √(aT)) and fills them to T = (Q − a/4)²/a; with u refitted to ≈ 113 km/s X-COP fits better (rms 0.203, trend 0.15) and galaxy lensing holds (gaps 0.183 / 0.198). **Trapped model on the full suite** (registered, README §36.9): with each galaxy's own glow counted, cluster galaxies hold only a quarter of theirs, uniformly, so X-COP keeps its shape (u refits to 146 km/s); own-heat reading 57 / 12 / 8 against 59 / 11 / 7 (SPARC bulges, the Milky Way at 15–27 kpc, Mistele's ellipticals lose a grade), cluster-heat reading 57 / 9 / 11; not adopted, and the costs sit where the fill-up acts. Next: the complete trapped model (trapping + fill-up, a and g_d refitted), registered; a registered no-hold test on new dwarfs (M31's); weigh the lenses' circumgalactic gas. |
