...
## Status — 24 September 2026 (hot-companion rows updated during round 16)
...
**Next, after round 15 (hot-companion gravity):** results README §25.5 and BLOG §9.

1. **A physical quiet store with a gap:** an internal pair of oscillations that relative motion detunes (the motional
   mixing of a metastable atomic state is the laboratory analogue); derive χ and γ, and so u = √(γ₀γ)/χ.
2. **One kind of matter:** every piece both source (a quiet store opened by motion) and receiver (inverted,
   self-sustained), in one cloud with the mutual waves fed back.
