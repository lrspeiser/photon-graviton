...
- **RUT-1: a written track supports an orbit, but survival buys width, not memory** (protocol 390bc37). The owner's wagon-rut footprint model, run in parallel with PM-2A because it asks whether moving matter can *write* attraction rather than whether a static equation predicts it. PM-3's theorem does not cover it: a steady writing pattern leaves Φ = −τ·(pattern), not zero.
  ...
  - **Stage 2 is the result.** The self-force on a prescribed circular orbit collapses exactly to one revolution times a geometric sum, reproducing A(N) = A_∞[1−e^{−N/N_build}] identically. Support accumulates in proportion to the memory time; the drag comes from the passage just made and is independent of it, so drag/support = C/(N_build·w/R) with C ≈ 0.59–0.78. **The memory time then cancels from survival:** L-change/build = (w/R)/(2πCf), so the orbit outlives its own build only if w/R > 4.4·f. Ten percent support demands a track 44% of the orbital radius — an annulus comparable to the orbit, not a rut, and at that width hard to distinguish from a smooth axisymmetric mass, which returns it to PM-3's steady-state theorem. One percent support needs 4%, which is a genuine track.
