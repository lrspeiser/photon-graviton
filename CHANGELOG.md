...
- **RUT-1 stage 5: the lag did not cause the instability -- it slowed it by a factor of fifty, and the mode is now predicted rather than observed** (protocol e354e65, in its own file so stage 4's archive stays frozen). After the owner's review of 474a5df, which supplied the control stage 4 was missing.
  ...
  - **No stable supported window, and not discreteness.** Growth is monotonic in support (28.2, 11.7, 8.58, 2.45 T0 at 2.5%, 5%, 10%, 20%) and unchanged from 8 to 128 writers at fixed total rate. The exactly cold ring grows the same mode at the same rate from the numerical floor, so the 2% jitter sets only when it becomes visible.
