...
The first command runs 49 jobs. The last four regenerate the MOND-guided SPARC, Milky Way cross-scale and Coma inverse diagnostics into the run directory and compare them numerically with their archived results (they never overwrite the archive without `--canonical`), then run the capture-to-orbit verification checks. The first 45 are: catalog no-loss comparison, closed and open energy ledgers, internal/external spatial capture, microphysical kinematics, frequency transfer, scalar-wave evolution, gravity/lensing response, the data/requirement audit, the conversion-first empirical pass, interaction-rate constraints, matter-assisted conversion, collective spatial response, oscillator frequency response, ordinary soft-graviton emission, reversible capture/storage, mechanical deposit support, capture momentum, moving-receiver isotropic capture, broadband receiver response, joined source/receiver spectral kinetics, dynamical matched electromagnetic waves, thermal conversion with an exploratory FIRAS comparison, background replenishment source/energy requirements, heated thermal absorption/emission with an exploratory FIRAS comparison, a joint slow-companion energy/angle kernel, an action-derived dispersive companion emission band, electric/magnetic polarization and color-cancellation limits, populated-companion reverse transfer/noise, cumulative whole-signal time stretching with companion-energy transfer, a closed photon/companion-field backreaction and momentum model, weak-signal carrier/event timing in that field, environmental field suppression versus companion-wave transmission, dynamically generated companion-wave access to a restoring region, source-timescale versus delivered-energy tradeoffs, multiple-region redshift with weak-signal event timing in one shared field, finite radiation-train mean stretching, drift and sampled variation, longer sustained-illumination comparisons with a derived rolling branch and in-flight energy accounting, spherical photon/field propagation with finite emitter fuel and a static-profile comparison, conditional companion self-binding with capture-energy and periodic arrival-map checks, joined bound-cloud/photon energy exchange with an occupation-symmetry constraint, neutral bound-pair production with recoil, inverse dynamics and a color-dependence diagnostic, and common pair creation/inverse-scattering/radiative-loss rates with finite illumination and dark population histories. The second adds the historical baseline's three scripts as one additional job, run in an isolated copy. Each job logs its output, and the runner checks that saved results and the 271 recovered original files stay unchanged.
...
The [companion-self-binding diagnostic](results/companion-self-binding/report.md) solves a conditional massive-wave stationary state and separately tests static/periodic optical response. It preserves an initial failed optical integration and refines the solver without changing the physical profile or acceptance gate. It does not claim a joined photon-binding model or an ordinary-graviton bound state.

[Bound-cloud energy exchange](results/bound-cloud-exchange/report.md). The density-dependent photon interaction transfers energy into breathing motion of a preexisting bound cloud but exactly conserves companion occupation. Ten refined Gaussian-cloud runs balance photon loss and cloud gain; this does not establish creation, capture, lifetime or a spin-2 identity. Next derive a production interaction with charge/occupation, energy and momentum accounting, retaining clocks, void coupling, cumulative redshift, source supply and joint rotation/lensing within all 20 tasks and 32 requirements.

[Bound-pair production](results/bound-pair-production/report.md). A conditional neutral scalar-pair interaction can populate initially empty bound modes while conserving charge, photon count, momentum and total energy including store recoil. It requires a seeded companion store and stipulated mode overlap. Twelve finite-mode runs retain reverse conversion; 180 kinematic checks and a fixed-transition rate calculation expose color dependence. This does not establish permanent deposition or whole-event stretching. Next derive spatial production/receiving modes and their common propagation law, preserving seed supply, clocks, weak-gravity coupling, halo support, joint rotation/lensing and all 20 tasks/32 requirements.

[Pair production and loss balance](results/pair-production-balance/report.md). A common point-response rate calculation includes neutral-pair creation, inverse photon scattering and two-photon loss. Twenty-four illumination histories and twenty-four dark histories agree with a finite stationary population and explicit energy accounts. Small gaps can suppress spontaneous loss at fixed coupling, but steady illumination does not supply indefinite single-mode growth. This is not a galaxy capacity or lifetime result. Next derive spatial modes and coherent forward response, retaining seed supply, clocks, weak-gravity coupling, spectral/timing tests, halo support, joint rotation/lensing and all 20 tasks/32 requirements.
