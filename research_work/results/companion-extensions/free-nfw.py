...
shapes={v:json.loads((P/f"nfw-shape-{v:g}-results.json").read_text()) for v in [.3,1.,3.]}
...
out=dict(scope="Bounded NFW scale and strength plus four stellar parameters fitted to all motion bins; lens calibration consumed; no independent prediction or stability proof",rows=[])
...
for old in source['rows']:
    ...
    def unit_deflection(log_rs):
        rs=Re*np.exp(log_rs)
        def integrand(t):
            rr=impact/np.cos(t)
            return float(nfw_fraction(rr/rs))/rr
        return 4*G/C**2*quad(integrand,0,np.pi/2,epsabs=1e-13,epsrel=1e-11,limit=250)[0]
    rs_grid=np.linspace(np.log(.1),np.log(100),257)
    deflection_table=PchipInterpolator(rs_grid,np.log([unit_deflection(v) for v in rs_grid]))
    def cap(bi,log_ra):
    ...
    def evaluate(q,direct=False):
        ...
        unit=unit_deflection(log_rs) if direct else float(np.exp(deflection_table(log_rs)))
        amplitude=required*f/unit
        force=mass/1e11*((1-weight)*model.forces[0]+weight*model.forces[1])+G*amplitude*nfw_fraction(r/rs)/r**2
    ...
    seeds=[];reproduction=[]
    for ratio,run in shapes.items():
        for row in run['rows']:
            if row['Name']!=name:continue
            log_rs=np.log(row['nfw_rs_kpc']/Re)
            f=row['nfw_mass_amplitude_Msun']*unit_deflection(log_rs)/required
            q=[row['h'],row['beta0'],row['beta_infinity'],np.log(row['orbit_transition_radius_Re']),log_rs,f]
            err=abs(evaluate(q,True)[0]-row['all_motion_chi2']);assert err<1e-5
            reproduction.append(err)
            assert .1<=np.exp(log_rs)<=100 and 0<=f<=.95
            seeds.append(q)
...
(P/'free-nfw-results.json').write_text(json.dumps(out,indent=2,allow_nan=False)+'\n',encoding='utf-8',newline='\n')
