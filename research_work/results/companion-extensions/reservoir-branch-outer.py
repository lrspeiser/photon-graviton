"""Frozen outer comparison of seven J1621 fixed-scale branches."""
...
branches=json.loads((P/'reservoir-scale-scan-results.json').read_text())
template=next(v for v in source['rows'] if v['Name']=='J1621+3931' and v['retention_mapping']['population']=='Chabrier')
for branch in branches['rows']:
    old=template
    ...
    fitted=dict(branch,stored_density_normalization_Msun_kpc3=branch['density_Msun_kpc3'],outer_grid_radius_in_capture_scales=r[-1]/branch['capture_scale_kpc'],stored_mass_within_grid_Msun=branch['stored_mass_Msun'])
    ...
    assert consistency<1e-5
    assert max(abs(pr['kappa']-coarse['kappa']))<1e-6
    assert max(abs(pr['mean_kappa']-coarse['mean_kappa']))<1e-6
    out['rows'].append(dict(Name=name,capture_scale_Re=branch['capture_scale_Re'],inner_fit_all_motion_chi2=branch['all_motion_chi2'],capture_scale_kpc=ac,sigma_critical_Msun_kpc2=sigma_crit,source_geometry=old['geometry'],catalogue_lens_radius_kpc=impact,catalogue_lens_mean_kappa_error=float(lens_error),mass_grid_upper_radius_kpc=Rmax,mass_inside_grid_Msun=float(mc[-1]),relative_mass_difference_from_fit_grid=float(mc[-1]/fitted['stored_mass_within_grid_Msun']-1) if fitted['stored_mass_within_grid_Msun'] else 0.0,outer_predictions=[dict(radius_kpc=float(b),angle_arcsec=float(b/dl*ARCSEC),circular_speed_kms=float(v),stellar_only_circular_speed_kms=float(np.sqrt(G*star_enclosed(b)/b)),companion_enclosed_Msun=float(comp_enclosed(b)),**{k:float(vv[i]) for k,vv in pr.items()}) for i,(b,v) in enumerate(zip(radii,speed))],central_potential_magnitude_over_c2_within_grid=float(potential_comp+potential_star),exterior_companion_potential_bound_over_c2=float(potential_tail_bound),peak_scanned_circular_speed_kms=float(vscan[peak]),peak_speed_radius_kpc=float(scan[peak]),peak_scanned_compactness_2GM_rc2=float(2*max(vscan*vscan)/C**2),surface_projection_derivative_max_kappa_difference=consistency,quadrature_max_absolute_kappa_difference=float(max(abs(pr['kappa']-coarse['kappa']))),quadrature_max_absolute_mean_kappa_difference=float(max(abs(pr['mean_kappa']-coarse['mean_kappa'])))))
    print(name,'peak vc',vscan[peak],'lens error',lens_error,'projection check',consistency,flush=True)
prior=json.loads((P/'outer-companion-predictions-results.json').read_text())
prior=next(v for v in prior['rows'] if v['Name']=='J1621+3931')
last=out['rows'][-1]
velocity_difference=max(abs(v['circular_speed_kms']-u['circular_speed_kms']) for v,u in zip(last['outer_predictions'],prior['outer_predictions']))
shear_difference=max(abs(v['reduced_shear']-u['reduced_shear']) for v,u in zip(last['outer_predictions'],prior['outer_predictions']))
assert velocity_difference<.01 and shear_difference<1e-6
out['extreme_branch_reproduction']=dict(max_speed_difference_kms=velocity_difference,max_reduced_shear_difference=shear_difference)
...
out['input_sha256']={str(f.relative_to(ROOT)).replace('\\','/'):hashlib.sha256(f.read_bytes()).hexdigest() for f in [P/'outer-companion-predictions-results.json',P/'free-companion-results.json',P/'reservoir-branch-outer-protocol.md',P/'reservoir-scale-scan-results.json',Path(__file__),L/'capacity-reference-optics-results.json',P.parent/'slacs-light-profile-audit/results.json',P.parent/'slacs-component-refit/model.py',P.parent/'slacs-resolved-fit/model.py',P.parent/'slacs-resolved-input-audit/results.json',P.parent/'slacs-motion-lensing-pilot/results.json',P.parent/'lensing-data-readiness/conditional-geometry.json',L/'third-radiation-retention-results.json']}
(P/'reservoir-branch-outer-results.json').write_text(json.dumps(out,indent=2,allow_nan=False)+'\n',encoding='utf-8',newline='\n')
