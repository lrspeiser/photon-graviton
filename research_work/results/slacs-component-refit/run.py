...
(HERE/'results.json').write_text(json.dumps(dict(scope=protocol['scope'],rows=rows,excluded=excluded,hashes={str(f.relative_to(ROOT)):hashlib.sha256(f.read_bytes()).hexdigest() for f in paths+[HERE/'run.py',HERE/'model.py',PILOT/'model.py',HERE.parent/'slacs-resolved-fit/model.py']}),indent=2)+'\n',encoding='utf-8',newline='\n')
