...
for cells in [80,160,320]:
    ...
    for label,epsilon,strength,tau in cases:
        ...
        for epoch in [20.,30.,40.]:
            nt=rhs(epoch,sol.sol(epoch))[:-5].reshape(4,cells)[2]/K
            history.append(dict(time=epoch,max_abs_n_t=float(max(abs(nt)))))
        n30=1+sol.sol(30)[:-5].reshape(4,cells)[2]/K
        result=dict(cells=cells,case=label,response_time=tau,history=history,n_change_30_to_40=float(max(abs(n[:,-1]-n30))),epsilon=epsilon,capture_strength=strength,minimum_energy=float(f.min()),min_d=float(d.min()),min_speed_gap=float((n*d-sigma).min()),
...
(HERE/'results.json').write_text(json.dumps(dict(runs=runs,refinement=refinement,scope='Slow-response sensitivity with fixed photon initiation and capture; no observational fit or universal stationarity proof.'),indent=2)+'\n',encoding='utf8',newline='\n')
