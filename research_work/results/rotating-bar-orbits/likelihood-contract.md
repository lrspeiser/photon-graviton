...
The existing StarHorse files contain posterior distance summaries, not a standalone measurement likelihood. A hierarchical fit must use appropriate posterior-recycling prior corrections where known, or label its uncertainty integration as approximate. Never multiply the same Gaia parallax likelihood into StarHorse distances a second time. The existing four uncertainty reconstructions are sensitivity inputs; they are not an exact survey likelihood.
