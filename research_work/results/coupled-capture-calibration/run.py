...
class Model:
    def __init__(self,nr,na,lmax):
        ...
        self.r=r=np.geomspace(1e-4,100,nr);mu,w=roots_legendre(na);self.ells=ells=np.arange(0,lmax+1,2)
...
for p in [2,3]:
    ...
    fits.append(dict(p=p,scan=scan,optimizers=optimizers,edge_optimum=edge,best=best,sensitivity=sensitivity,timecheck=timecheck))
    print(json.dumps(dict(stage='fit',p=p,best=best)),flush=True)
    (HERE/'calibration.json').write_text(json.dumps(fits,indent=2)+'\n',encoding='utf8',newline='\n')
del coarse
...
out=dict(fits=fits,checks=checks,observed_training_proxy=observed.tolist(),ordinary_prediction=np.sqrt(base).tolist(),
    hashes={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in [src,prior_path,star_path]},
    scope='One-parameter training calibration of chronological capture feedback at fixed100kpc cutoff; no new holdout or complete energy/clock model.')
