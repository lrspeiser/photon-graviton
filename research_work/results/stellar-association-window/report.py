...
import numpy as np
r=json.loads((HERE/'results.json').read_text()); c=json.loads((HERE/'confirmation.json').read_text())
rows=[]
for name,v in r['regions'].items():
    before=v['sensitivity'][0];after=v['sensitivity'][2]
    rows.append(f"| {name.replace('_',' ')} | {v['stars']} | {v['flagged_0.5_arcsec']} | {before['stds_kms'][2]:.2f} | {after['stds_kms'][2]:.2f} |")
text='''# Bulge comparison: positional-association sensitivity
...
'''+f"Of {r['training_stars']:,} training rows, {r['epoch_screen_applicable_training_stars']:,} have applicable 2MASS identifiers and {r['non_2mass_identifiers_not_epoch_screened']:,} do not. The 0.5-arcsec screen flags 140 applicable rows, including 35 missed by the previous unlimited-epoch criterion. Allowing arbitrary centuries can falsely reconcile offset positions with measured motion. All 140 exact designation queries returned one record each; their actual-epoch separations range upward from {c['min_actual_epoch_residual_arcsec']:.3f} arcsec.\n\n"+'''
...
names=list(r['regions']); x=np.arange(len(names)); fig,ax=plt.subplots(figsize=(9,4.5))
ax.bar(x-.18,[r['regions'][k]['sensitivity'][0]['stds_kms'][2] for k in names],.36,label='Original training sample')
ax.bar(x+.18,[r['regions'][k]['sensitivity'][2]['stds_kms'][2] for k in names],.36,label='Position flags omitted: sensitivity only')
ax.set_xticks(x,[k.replace('_','\n') for k in names]);ax.set_ylabel('Descriptive vertical-speed spread (km/s)')
ax.set_title('Catalog association can change the apparent bulge-height contrast')
ax.legend(fontsize=9);fig.tight_layout();fig.savefig(HERE/'comparison.png',dpi=160);plt.close(fig)
