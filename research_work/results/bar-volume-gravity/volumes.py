...
n=int(sys.argv[1]);layers=int(sys.argv[2]);assert n in (6,8) and layers in (64,128,256,512)
d=json.loads((HERE/f'prepared{n}.json').read_text(encoding='utf8'));assert len(d['records'])==d['expected']==n*n
assert all(r['passes'] for r in d['records'])
out=[]
for R in (1.,3.):
    group=[r for r in d['records'] if r['R']==R]
    directions=[];captures=[];splines=[]
    for r in group:
        ...
        directions.extend(np.array(r['direction'])*SYMMETRIES)
        captures.extend([r['capture']]*4)
    directions=np.array(directions);captures=np.array(captures)
    faces=np.sort(ConvexHull(directions).simplices,axis=1)
    ...
    for T in (.05,.1,.25):
        ...
        x=np.concatenate([s(ages)[:,None,:]*SYMMETRIES[None,:,:] for s in splines],axis=1)
        ...
        (HERE/f'volume-n{n}-t{layers}.json').write_text(json.dumps(dict(points=POINTS.tolist(),records=out),indent=2)+'\n',encoding='utf8',newline='\n')
