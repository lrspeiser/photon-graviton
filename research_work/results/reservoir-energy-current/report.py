"""Derive local currents for the stipulated packet ensemble and report controls."""
...
lines=[
    ...
    '|---|---:|---:|',
    f"| Initial total energy, including seed | {r['initial_total_energy']:.6f} | {r['initial_total_energy']:.6f} |",
    f"| Acquired reservoir energy, entire domain | {r['final_moving_reservoir_gain']:.6f} | {r['final_stationary_reservoir_gain']:.6f} |",
    f"| Acquired energy remaining left of x=5 | {r['moving_gain_left_of_x5']:.2g} | {r['stationary_gain_left_of_x5']:.6f} |",
    f"| Total energy transported through x=5 | {r['moving_total_energy_crossed_x5']:.6f} | {r['stationary_total_energy_crossed_x5']:.6f} |", '',
...
for q in d['runs']:
    lines.append(f"| {q['cells']} | {q['number_density_L1_error']:.5f} | {q['stationary_control_error']:.6g} | {q['maximum_control_volume_error']:.3g} |")
lines += ['',
...
print('Energy-current report written.')
