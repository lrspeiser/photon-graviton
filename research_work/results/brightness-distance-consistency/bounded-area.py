...
def pred(eta):return baseline+2.5*np.log10(1+eta*f)-float(w@(2.5*np.log10(1+eta*fcal)))
def score(res,factor):return float(res@cho_solve(factor,res))
def loss(eta):return score((obs-pred(eta))[train],fac)
opt=minimize_scalar(loss,bounds=(0,10),method='bounded',options={'xatol':1e-10})
eta=min([0.,10.,float(opt.x)],key=loss);prediction=pred(eta)
assert abs(loss(0)-old['train_baseline_chi2'])<1e-7
...
out=dict(status='Exposed standardized-data fit; postulated optical response, not a derived metric',eta=eta,boundary=eta in [0,10],optimizer_success=bool(opt.success),alpha_per_mpc=alpha,b=1,train_baseline_score=loss(0),train_revised_score=loss(eta),farther_baseline_score=score((obs-baseline)[test],testfac),farther_revised_score=score((obs-prediction)[test],testfac),M_calibrated=M0-float(w@(2.5*np.log10(1+eta*fcal))),area_ratio_at_z1=1+eta/2,maximum_area_ratio=1+eta,bins=bins)
