...
**Subsequent correction:** [Input flags and training-orbit support](../stellar-orbit-support/report.md) found that the earlier statement that StarHorse had already used Gaia parallax does not apply to every star. Its input flags omit PARALLAX for 19,381 training sources. Those cases require separate likelihood/quality treatment; some show large distance/parallax disagreement. The four calculations below remain documented sensitivity reconstructions, not repaired joint posteriors. The parent catalog and holdout roles are preserved, and no held-out gravity scores had been opened. Future stellar inference must use the recovered provenance/fidelity sidecar rather than apply a single parallax-reuse assumption to all rows.
