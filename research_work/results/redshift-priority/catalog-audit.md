...
The [distance-adoption and selection audit](elves-selection-report.md) confirms source priority TRGB > SBF > redshift rather than a described blend, clarifies that the first exported SBF flag is not trustworthiness, and records source-assigned versus measured uncertainty. A Gaussian toy calculation quantifies how lower-bound selection can skew accepted distance errors. It is not an ELVES correction or galaxy-motion model. The 24 pending entries comprise nine TRGB and 15 SBF targets.
