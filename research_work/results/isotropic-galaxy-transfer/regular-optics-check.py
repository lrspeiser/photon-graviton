...
prefix='relaxing' if relaxing else 'regular'
fit=json.loads((HERE.parent/f'brightness-distance-consistency/{prefix}-area-results.json').read_text())
result=json.loads((HERE/f'{prefix}-optics-results.json').read_text())
f,q=sp.symbols('f q',positive=True)
area=1+f*sp.exp(-q*f) if relaxing else 1+f/(1+q*f)
H=-(1-f)*sp.log(1-f)*sp.sqrt(area)
...
(HERE/f'{prefix}-optics-check.json').write_text(json.dumps(rows,indent=2)+'\n',encoding='utf-8')
