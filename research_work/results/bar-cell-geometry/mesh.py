"""One nested geodesic subdivision; new directions need actual trajectories."""
...
if __name__=='__main__':
    d=json.loads((HERE.parent/'bar-volume-gravity/prepared6.json').read_text(encoding='utf8'))
    original=[r for r in d['records'] if r['R']==1]
    vertices=[x.tolist() for r in original for x in np.array(r['direction'])*SYM]
    parent=np.sort(ConvexHull(np.array(vertices)).simplices,axis=1)
    ...
    out=dict(vertices=vertices.tolist(),faces=faces.tolist(),parent_faces=parent.tolist(),
             ...
             solid_angle=float(sum(area)),parent_vertices=len(original)*4)
    (HERE/'mesh.json').write_text(json.dumps(out,indent=2)+'\n',encoding='utf8',newline='\n')
