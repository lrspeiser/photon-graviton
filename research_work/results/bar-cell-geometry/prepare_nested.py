...
R=float(sys.argv[1]);assert R in (1.,3.)
mesh=json.loads((HERE/'mesh.json').read_text(encoding='utf8'))
previous=json.loads((HERE.parent/'bar-volume-gravity/prepared6.json').read_text(encoding='utf8'))
...
for i,direction in enumerate(np.array(mesh['directions'])):
    ...
    if reused:
        ...
        target=cache/f'nested1-R{R:g}-orbit{i:03d}.npz'
    ...
    records.append(row)
    (HERE/f'prepared-R{R:g}.json').write_text(json.dumps(dict(R=R,expected=len(mesh['directions']),field_hash=field_hash,
        mesh_hash=hashlib.sha256((HERE/'mesh.json').read_bytes()).hexdigest(),records=records),indent=2)+'\n',encoding='utf8',newline='\n')
