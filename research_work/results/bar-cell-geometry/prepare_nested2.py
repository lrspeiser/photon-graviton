...
previous=json.loads((HERE/f'prepared-R{R:g}.json').read_text(encoding='utf8'))
...
for i,direction in enumerate(np.array(mesh['directions'])):
    ...
    if reused:
        ...
        target=cache/f'nested2-R{R:g}-orbit{i:03d}.npz'
    ...
    records.append(row)
    (HERE/f'prepared2-R{R:g}.json').write_text(json.dumps(dict(R=R,expected=len(mesh['directions']),field_hash=field_hash,
        mesh_hash=hashlib.sha256((HERE/'mesh2.json').read_bytes()).hexdigest(),records=records),indent=2)+'\n',encoding='utf8',newline='\n')
