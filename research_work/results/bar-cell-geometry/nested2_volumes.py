...
d=json.loads((HERE/f'prepared2-R{launch_R:g}.json').read_text(encoding='utf8'))
assert len(d['records'])==d['expected']==len(mesh['directions'])
assert all(r['passes'] for r in d['records'])
assert d['mesh_hash']==hashlib.sha256((HERE/'mesh2.json').read_bytes()).hexdigest()
out=[]
for R in (launch_R,):
    ...
    for T in selected_times:
        ...
        (HERE/f'volume2-t{layers}-R{R:g}.json').write_text(json.dumps(dict(points=POINTS.tolist(),records=out),indent=2)+'\n',encoding='utf8',newline='\n')
