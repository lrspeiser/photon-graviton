...
results=[]
selected=(float(sys.argv[1]),) if len(sys.argv)>1 else (3.,)
assert selected==(3.,)
for R in selected:
    d=json.loads((HERE/f'prepared-adaptive-R{R:g}.json').read_text(encoding='utf8'))
...
(HERE/('adaptive-geometry-R3.json')).write_text(json.dumps(results,indent=2)+'\n',encoding='utf8',newline='\n')
