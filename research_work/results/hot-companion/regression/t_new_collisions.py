...
def macs(law, ctx):
    ...
    spec = CV.macs0025()
    ctx.log('MACS J0025.4-1222: lensing map (t = 0.26 Gyr)')
    sol = CV.solve(spec, law, 0.26)
    ...
    out.append(make('macs0025.sigma', GROUP, 'MACS J0025: galaxies\' speed spread (line of sight, inside 1 Mpc)', m['sigma_los_1Mpc'],
                    crit=z_check(m['sigma_los_1Mpc'], 835.0, 59.0), unit='km/s', target='835 +- 59 (108 galaxies)', refs=ref))
...
def a520(law, ctx):
    import collisions_v8 as CV
    spec = CV.abell520()
    ctx.log('Abell 520: lensing map (t = 0.5 Gyr)')
    ...
    clowe = dict(P1=(3.03, 0.69), P2=(4.08, 0.73), P3=(2.26, 0.75), P4=(4.64, 0.63), P5=(3.00, 0.77), P6=(3.03, 0.66))
    out = []
    for k in ('P1', 'P2', 'P3', 'P4', 'P5', 'P6'):
        ...
        out.append(make(f'abell520.m150_{k.lower()}', GROUP, f'Abell 520 {k}, {what}: lensing mass inside 150 kpc', m[f'M150_{k}'],
                        ...
                        refs='Jee et al. 2014 Table 1; Clowe et al. 2012 Table 1'))
...
def gordo(law, ctx):
    import collisions_v8 as CV
    spec = CV.el_gordo()
    ctx.log('El Gordo: lensing map (t = 0.91 Gyr, the returning phase)')
    sol = CV.solve(spec, law, 0.91)
    ...
    out.append(make('elgordo.peak_se', GROUP, 'El Gordo SE: lensing peak distance from the cool gas core', p['to_cool_core'],
                    crit=range_check(p['to_cool_core'], 62.0, 100.0, 25.0), unit='kpc',
                    target='about 8 arcsec (62-100 kpc), on the merger-centre side (Jee et al. 2014; Ng et al. 2015)',
