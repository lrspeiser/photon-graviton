...
Keys of the law dictionary (code units: kpc, km/s, Msun; accelerations in (km/s)^2/kpc):
    ...
    release_length_au   candidate amendment (round 7): the companion is released gradually over a
                        length L around each emitter, R(r) = 1 - exp(-r/L); 0 = at once (the law as is)
...
'u' on X-COP), and explicit overrides a_SI / lam / u_kms.
"""
...
def load_law(spec=None):
    """spec: None (the round-3 law), a candidate name in candidates/, or a path to a JSON file."""
    if spec in (None, '', 'round3'):
        c = base_constants()
        return make_law(c['a_code'], c['lam'], c['u_kms'], name='round3',
                        description='the round-3 law with its published constants (run-v3)')
    p = Path(spec)
    if not p.exists():
    ...
    return make_law(a, lam * gs, u, name=cfg.get('name', p.stem), description=cfg.get('description', ''),
                    release_length_au=cfg.get('release_length_au', 0.0), external_hold=cfg.get('external_hold', 1.0),
