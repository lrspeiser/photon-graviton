...
runs=[x for x in r['runs'] if x['quadrature_nodes']==32]
rows=[]
for x in runs:
 rows.append(f"| {x['signal_energy']:g} | {x['mean_carrier_stretch']:.6f} | {x['count_weighted_duration_ratio']:.6f} | {x['energy_weighted_duration_ratio']:.6f} | {100*x['affine_residual_rms_over_received_width']:.4f}% |")
text='''# Does the signal-feedback prediction depend on numerical packet grouping?
...
fig,ax=plt.subplots(1,2,figsize=(10,4.3));pos=np.arange(len(runs));labels=[f"{x['signal_energy']:g}" for x in runs]
ax[0].plot(pos,[x['energy_weighted_duration_ratio'] for x in runs],'o-',label='Energy-weighted duration')
ax[0].plot(pos,[x['mean_carrier_stretch'] for x in runs],'x--',label='Mean carrier stretch')
ax[0].set_ylabel('Reference stretch factor');ax[0].legend(fontsize=8)
ax[1].plot(pos,[100*x['affine_residual_rms_over_received_width'] for x in runs],'o-',color='#c55135')
ax[1].set_ylabel('Non-affine arrival residual (% of width)')
for a in ax:a.set_xticks(pos,labels);a.set_xlabel('Total signal energy (model units)')
fig.suptitle('One smooth pulse: brightness dependence survives numerical refinement')
