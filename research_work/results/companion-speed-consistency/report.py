...
import json
HERE=Path(__file__).resolve().parent;r=json.loads((HERE/'results.json').read_text());m=json.loads((HERE/'matched-speed-check.json').read_text())
rows=[]
for x in r['runs']:
 if x['modes']!=64:continue
 lo,hi=x['sampled_companion_to_photon_speed_ratio']
 rows.append(f"| {x['signal_energy']:g} | {x['wave_speed']:.2f} | {lo:.4f} - {hi:.4f} | {x['mean_carrier_stretch']:.6f} | {x['energy_weighted_duration_ratio']:.6f} |")
text='''# Companion speed must track the optical environment
