...
def solve(energy,nodes,modes=32,split=1,wave_speed=.5):
    ...
    L=8.;mass0=8.;mass=4.;v=wave_speed
    ...
    totals=[];momenta=[];field=[];bound=[];optical_on_path=[]
    for t in np.linspace(0,duration,181):
        ...
        positions=y[off+1:off+N]
        valid=(positions>=.4)&(positions<=2.4)
        if valid.any():optical_on_path.extend(optical(y,positions[valid])[0].tolist())
    ...
    summary={'signal_energy':energy,'quadrature_nodes':nodes,'coincident_split':split,'modes':modes,
       'wave_speed':v,'sampled_optical_factor_on_signal_path':[min(optical_on_path),max(optical_on_path)],
       'sampled_companion_to_photon_speed_ratio':[v*min(optical_on_path),v*max(optical_on_path)],
