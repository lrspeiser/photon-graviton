"""Explain the conditional loading-shape test and retain restricted-grid checks."""
...
rows=d['rows'];q=np.array([r['required_extra_per_baryon'] for r in rows])
subsets={
    'full_grid':np.ones(len(rows),dtype=bool),
    'inner_sampled_region_R_le_8_z_le_1':np.array([r['R_kpc']<=8 and r['z_kpc']<=1 for r in rows]),
    'midplane_only':np.array([r['z_kpc']==0 for r in rows]),
}
checks={}
for name,mask in subsets.items():
    lo,hi=float(q[mask].min()),float(q[mask].max())
    checks[name]=dict(points=int(mask.sum()),minimum=lo,maximum=hi,range_factor=hi/lo,
                      minimum_worst_fractional_error=(hi-lo)/(hi+lo),
                      minimum_worst_multiplicative_factor=float(np.sqrt(hi/lo)))
(HERE/'assessment.json').write_text(json.dumps(dict(subsets=checks,
    interpretation='Conditional source-shape mismatch, not observational model exclusion.',
    results_sha256=run.digest(HERE/'results.json'),holdouts_opened=False),indent=2)+'\n',encoding='utf-8',newline='\n')
lines=[
...
for name,s in checks.items():
    lines.append(f"| {name.replace('_',' ')} | {s['points']} | {s['minimum']:.4f} | {s['maximum']:.4f} | {s['range_factor']:.2f} |")
lines += ['',
    ...
    f"The bound is {100*checks['full_grid']['minimum_worst_fractional_error']:.2f}% over the full grid and {100*checks['inner_sampled_region_R_le_8_z_le_1']['minimum_worst_fractional_error']:.2f}% over the inner subset. Under a symmetric multiplicative-error criterion, even the best constant must miss some full-grid target by a factor of at least {checks['full_grid']['minimum_worst_multiplicative_factor']:.2f}, or {checks['inner_sampled_region_R_le_8_z_le_1']['minimum_worst_multiplicative_factor']:.2f} on the inner subset. Different loss functions change the preferred constant; none makes unequal q values equal.", '',
