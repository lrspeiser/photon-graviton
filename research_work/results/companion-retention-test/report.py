...
HERE=Path(__file__).resolve().parent;r=json.loads((HERE/'results.json').read_text());rows=[]
selected=[x for x in r['runs'] if x['cells']==512]
for x in selected:
 rows.append(f"| {x['rolling_rate']:.2f} | {x['initial_wave_amplitude']:.3f} | {x['mean_field_final']:.6f} | {100*x['wave_energy_retained_fraction']:.4f}% | {100*x['homogeneous_adiabatic_retention_prediction']:.4f}% |")
text='''# Does the equal-speed companion field preserve traveling-wave energy?
...
for x in selected:
 if x['initial_wave_amplitude']!=.001:continue
 h=x['history'];ax.plot(h['time'],[100*v for v in h['wave_energy_fraction']],label=f"Initial background velocity {x['rolling_rate']:g}")
ax.set(xlabel='Reference time (model units)',ylabel='Traveling perturbation energy retained (%)',title='Equal-speed field: wave energy transfers into background motion')
ax.legend();fig.tight_layout();fig.savefig(HERE/'comparison.png',dpi=160);plt.close(fig)
