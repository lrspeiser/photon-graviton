...
1. **Improve and verify the orbit population.** The library has expanded from 72 to 108 passing short integrations. On the updated eligible training sample, joint position/velocity coverage increased from 16.89% to 50.50% at the illustrative 0.5-kpc / 50-km/s matching resolution, but falls to 8.55% using only the later half of the paths. Those thresholds are diagnostic choices, not error bars. Subsequent paired long integrations and common-weight optimization show that the tested eight-path subset does not provide a stable joint position/velocity population across the tested time windows. These are limitations of the current numerical representation, not a rejection or validation of either gravity model. See the orbit-library-expansion, paired-orbit-duration and orbit-mixture-stability reports. Missing kinds and phases of orbit cannot be supplied merely by adjusting their weights.
