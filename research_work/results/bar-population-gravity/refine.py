...
grid='fine';order=40
original=json.loads((HERE.parent/'bar-angular-refinement/grid8-L40.json').read_text())
...
for i,r in enumerate(original['records']):
    ...
    records.append(row)
    print(i+1,64,row['passes'],flush=True)
    (HERE/'orbits8.json').write_text(json.dumps(dict(records=records,expected=64,source_hash=original['source_hash'],
