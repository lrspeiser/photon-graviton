...
tables=[];raw_checks=[]
datasets=[(8,json.loads((HERE/'orbits8.json').read_text()))]
parts=[json.loads((HERE/f'orbits12-R{R}.json').read_text()) for R in (1,3)]
for R,part in zip((1,3),parts):
    assert len(part['records'])==part['expected']==72
    assert all(r['R']==R for r in part['records'])
assert parts[0]['points']==parts[1]['points'] and parts[0]['source_hash']==parts[1]['source_hash']==datasets[0][1]['source_hash']
datasets.append((12,dict(records=parts[0]['records']+parts[1]['records'],points=parts[0]['points'],expected=144)))
for n,d in datasets:
...
comparisons=[]
for b in [r for r in tables if r['n']==12]:
    a=next(r for r in tables if r['n']==8 and r['R']==b['R'] and r['T']==b['T'] and r['point']==b['point'])
...
(HERE/'source-refinement12.json').write_text(json.dumps(out,indent=2)+'\n',encoding='utf8',newline='\n')
...
with (HERE/'source-refinement12.csv').open('w',encoding='utf8',newline='') as f:
