...
grid='fine';order=40
original=json.loads((HERE.parent/'bar-source-persistence/audit-fine-L40.json').read_text())
...
for i,r in enumerate(original['records']):
    ...
    records.append(row)
    print(i+1,36,row['passes'],flush=True)
    (HERE/'orbits.json').write_text(json.dumps(dict(records=records,expected=36,source_hash=original['source_hash'],
