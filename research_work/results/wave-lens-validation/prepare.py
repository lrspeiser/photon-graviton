...
obs=[r for r in json.loads(obsfile.read_text()) if r['role']=='validation']
geo={r['Name']:r for r in json.loads(geofile.read_text()) if r['role']=='validation'}
...
for r in obs:
    ...
    rows.append({'Name':name,'role':'validation','imf':'Salpeter','model':'baryons',
...
assert len(rows)==7 and len(excluded)==2 and max(distance_errors)<1e-10
