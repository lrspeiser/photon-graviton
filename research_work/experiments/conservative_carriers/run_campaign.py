"""Execute the 252 SV-1 cases and 36 predeclared refinements."""
import os
...
def plain(v):
    ...
    return v
def save(p,v):p.write_text(json.dumps(plain(v),indent=2,allow_nan=False)+"\n",encoding="utf8",newline="\n")
def digest(p):return hashlib.sha256(p.read_bytes()).hexdigest()
def integrate(spec,fine=False):
    y,spin=M.initial(spec["packet"]);e0=M.energy(y,spec);comp=M.components(y,spec)
...
def main():
    ...
    controls,pairs=M.controls();save(out/"controls.json",controls);save(out/"pair-heading-probes.json",pairs)
    if not all(r["passed"] for r in controls):raise AssertionError("Preserved control failure")
    print(f"{len(controls)} Hamiltonian controls pass",flush=True)
    ...
    save(HERE/"evidence-sha256.json",{p.relative_to(HERE).as_posix():digest(p) for p in sorted(out.iterdir())})
