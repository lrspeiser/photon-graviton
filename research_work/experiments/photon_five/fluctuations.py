...
def system(n,correlation=0.):
    ...
    if correlation:
        tc=correlation
        Zm=Dm/(2*tc*(eig[:,None]+1/tc+1/tc**2))
        Xmodal=((1+1/tc)*(Zm+Zm.T)-.5*(eig[:,None]-eig[None,:])*(Zm-Zm.T))/(eig[:,None]+eig[None,:]+.5*(eig[:,None]-eig[None,:])**2)
        Z=U@Zm@U.T
    else: Z=np.zeros((n,n))
    X=U@Xmodal@U.T
    V=(K@X+X@K-Z-Z.T)/2
...
def covariance_checks(n=64,correlation=0.):
    r,dx,K,D,S,B=system(n,correlation)
    A=np.block([[np.zeros((n,n)),np.eye(n)],[-K,-np.eye(n)]])
    forcing=np.zeros_like(A); forcing[n:,n:]=D
    if correlation:
        Z=np.zeros((n,n));I=np.eye(n)
        A=np.block([[Z,I,Z],[-K,-I,I],[Z,Z,-I/correlation]])
        forcing=np.zeros_like(A);forcing[2*n:,2*n:]=D/correlation**2
    lyap=solve_continuous_lyapunov(A,-forcing)
    error=np.max(abs(lyap[:2*n,:2*n]-S))/np.max(abs(S))
    injection=np.trace(lyap[n:2*n,2*n:])*dx if correlation else .5*np.trace(D)*dx
...
def orbit(n,steps_per_orbit,average,seed,correlation=0.):
    r,dx,K,D,S,B=system(n,correlation)
    ...
    a=np.zeros(n); v=np.zeros(n); power=np.zeros(n); xi=np.zeros(n)
    ...
    for k in range(total):
        ...
        before=energy(a,v)
        if correlation:
            tc=correlation; e=np.exp(-dt/tc); one=-np.expm1(-dt/tc)
            variance_xi=(1-e*e)/(2*tc)
            variance_integral=dt-2*tc*one+tc*(1-e*e)/2
            covariance=one*one/2
            z1=B@rng.standard_normal(n);z2=B@rng.standard_normal(n)
            integral=tc*one*xi+covariance/np.sqrt(variance_xi)*z1
            integral+=np.sqrt(max(variance_integral-covariance**2/variance_xi,0))*z2
            xi=e*xi+np.sqrt(variance_xi)*z1
            v+=integral
        else:
    ...
    return dict(correlation_time=correlation,grid=n,steps_per_orbit=steps_per_orbit,average_periods=average,seed=seed,
...
def run(correlation=0.):
    checks=covariance_checks(correlation=correlation)
    orbits=[]
    for avg in [.1,1,10]:
        for seed in [1909,1910,1911]:
            row=orbit(64,1024,avg,seed,correlation);orbits.append(row)
            print('E3 orbit',avg,seed,row['quiet'],flush=True)
    refined=[orbit(128,2048,avg,1909,correlation) for avg in [.1,1,10]]
    ...
    return dict(experiment='E3-colored' if correlation else 'E3',correlation_time=correlation,
                covariance_sensitivities={str(t):covariance_checks(correlation=t) for t in [.05,.2,1]} if correlation else {},checks=checks,gates=gates,numerical_pass=all(gates.values()),
                mechanism_pass=all(x['quiet'] for x in orbits+refined),orbits=orbits,refined=refined,
                observational_status='Unresolved finite photon reservoir, microphysical noise generation, matter backreaction and real-galaxy profiles.')
