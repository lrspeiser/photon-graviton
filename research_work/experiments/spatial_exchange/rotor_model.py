"""SE-Rot2: finite vector rotor coupled to sampled curl A.
...
class ExchangeEvolution:
    def __init__(self,n=32,length=16.,radius=.9,chi=0.,mix=.5,excitation=.001,angle=0.,rotor_coupling=.4,spin=1,orbit_speed=.2):
        self.n=n;self.length=length;self.radius=radius;self.chi=chi;self.mix=mix;self.excitation=excitation
        self.rotor_coupling=rotor_coupling;self.spin=spin;self.orbit_speed=orbit_speed
    ...
    def unpack(self,y):
        ...
        Q=y[start+36:start+54].reshape(6,3);P=y[start+54:start+72].reshape(6,3)
        return f,pi,q,p,Q,P

    def initial(self):
        theta=np.arange(6)*np.pi/3;q=.7*np.column_stack((np.cos(theta),np.sin(theta),np.zeros(6)))
        p=self.orbit_speed*np.column_stack((-np.sin(theta),np.cos(theta),np.zeros(6)))
        Q=np.zeros((6,3));P=np.zeros_like(Q)
        Q[:,0]=np.sqrt(self.excitation if self.spin else 2*self.excitation)/self.Omega
        P[:,1]=self.spin*np.sqrt(self.excitation)
        return np.concatenate((np.zeros(2*self.nf),(q@self.rot.T).ravel(),(p@self.rot.T).ravel(),(Q@self.rot.T).ravel(),(P@self.rot.T).ravel(),[0.]))
    ...
    def evaluate(self,y,derivatives=True):
        ...
        Bgrid=self.curl(A);deposit=np.zeros_like(A) if derivatives else None
        ...
        velocity=np.zeros_like(p);force=np.zeros_like(q);Qdot=np.zeros_like(Q);Pdot=np.zeros_like(P);bbar=np.zeros_like(p);cbar=np.zeros(6);masses=np.zeros(6)
        for i in range(6):
            sl,w,dw=self.block(q[i]);vsl=(slice(None),)+sl
            B=np.sum(w*Bgrid[vsl],axis=(1,2,3));K=P[i]-self.rotor_coupling*np.cross(B,Q[i])
            m=self.m[i]+(np.dot(K,K)+self.Omega**2*np.dot(Q[i],Q[i]))/2;masses[i]=m;internal+=m-self.m[i]
            ...
            response=float(np.sum(w*al*m/e));reaction=response*self.rotor_coupling*np.cross(Q[i],K)
            matter+=float(np.sum(w*h));bbar[i]=np.sum(w*bb,axis=(1,2,3));cbar[i]=np.sum(w*zz)
            velocity[i]=p[i]*np.sum(w*al*zz/e)+bbar[i]
            force[i]=-np.sum(dw*h[...,None],axis=(0,1,2))+np.sum(dw*np.einsum('jxyz,j->xyz',Bgrid[vsl],reaction)[...,None],axis=(0,1,2))
            Qdot[i]=response*K;Pdot[i]=-response*(self.Omega**2*Q[i]+self.rotor_coupling*np.cross(B,K))
            if derivatives:
                ...
                deposit[vsl]+=reaction[:,None,None,None]*w/self.dv
        ...
        if derivatives:
            pd[1:4]+=self.curl(deposit)
            pd-=self.gamma*fd;loss=float(np.sum(self.gamma*fd*fd)*self.dv)
            result=np.concatenate((fd.ravel(),pd.ravel(),velocity.ravel(),force.ravel(),Qdot.ravel(),Pdot.ravel(),[loss]))
    ...
    def metrics(self,y):
        ...
        Q,P=self.unpack(y)[4:];internal_angular=np.cross(Q,P).sum(axis=0);angular+=internal_angular
        ...
        result.update(total=t['field']+t['matter'],ledger=t['field']+t['matter']+y[-1],absorbed=float(y[-1]),momentum=momentum,angular=angular,internal_angular=internal_angular,
