"""SE-PB matched point interpolation with same-stage background integration."""
import os
...
from probes import coefficients
from point_probes import evaluate,derivative
...
def configurations():
    common=dict(length=16.,radius=.9,dt=.01)
    variants=[]
    for n in (32,48,64):
        variants += [dict(name=f'control-n{n}',n=n,chi=0.,mix=0.,emitter_angle=0.),
                     dict(name=f'Y-n{n}',n=n,chi=200.,mix=.5,emitter_angle=np.pi/2)]
    variants += [dict(name='Y-time',n=32,chi=200.,mix=.5,emitter_angle=np.pi/2,dt=.005)]
...
def run(cfg,out):
    ...
    def stage(bg,pr):
        ...
        return model.rhs(bg),derivative(pr,masses,model.length,arrays)
    for step in range(steps+1):
        arrays=coefficients(model.unpack(y)[0]);vel=[];energy=[]
        for row,mass in zip(probes,masses):
            value=evaluate(row[:3],row[3:],mass,model.length,*arrays)
    ...
    reference=ROOT/'emitter-v1/time-refinement.npz' if cfg['name']=='Y-n32' else None
    replay=float(np.max(abs(y-np.load(reference)['final']))) if reference is not None and reference.exists() else None
...
def main():
    out=ROOT/'point-bundle-v1';out.mkdir(exist_ok=False)
    save(out/'manifest.json',dict(commit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),
         sources={n:hashlib.sha256((ROOT/n).read_bytes()).hexdigest() for n in ['run_point_bundles.py','point_probes.py','probes.py','emitter_model.py','run_emitter.py','point-bundle-protocol.md']},configurations=configurations()))
    rows=[]
    for cfg in configurations():
        print('start',cfg['name'],flush=True);rows.append(run(cfg,out))
    by={r['config']['name']:r for r in rows};comparisons=[]
    for emitter in ('control','Y'):
        coarse,middle,fine=[by[f'{emitter}-n{n}'] for n in (32,48,64)]
        difference=abs(middle['central_bend']-fine['central_bend'])/max(abs(fine['central_bend']),1e-8)
        previous=abs(coarse['central_bend']-middle['central_bend'])/max(abs(middle['central_bend']),1e-8)
        matrix=float(np.max(abs(np.array(middle['transport']['matrices'][-1])-fine['transport']['matrices'][-1])))
        comparisons.append(dict(name=emitter+'-space',bend_relative_difference=difference,previous_difference=previous,matrix_difference=matrix,passed=difference<.01 and difference<previous and matrix<.001))
    gain48=by['Y-n48']['central_bend']-by['control-n48']['central_bend']
    gain64=by['Y-n64']['central_bend']-by['control-n64']['central_bend']
    comparisons.append(dict(name='paired-effect',coarse_difference=gain48,fine_difference=gain64,passed=abs(gain64-gain48)<=max(.2*abs(gain64),1e-7)))
    base,fine=by['Y-n32'],by['Y-time']
    ...
    comparisons.append(dict(name='time',bend_relative_difference=bend,arrival_difference=delay,matrix_difference=matrix,passed=bend<.001 and delay<1e-5 and matrix<1e-4))
