"""Independent SE-2 energy reconstruction and honest partial/full audit."""
import os
...
def reconstruct(y,cfg):
    ...
    for i in range(6):
        ...
        theta=cfg.get('emitter_angle',0.);sampled_x=np.sum(weight*(np.cos(theta)*f[4]+np.sin(theta)*f[5]));mass=1+(P[i]**2+(Q[i]-emission*sampled_x)**2)/2;internal+=mass-1
...
def main():
    directory=ROOT/'emitter-v1';manifest=json.loads((directory/'manifest.json').read_text());controls=json.loads((directory/'controls.json').read_text())
    ...
    for row in controls['rows']:
        expected=row['equivalence_error']<1e-12 and row['gradient_error']<2e-6 and row['minimum_mass']>0 and row['cone_error']<1e-10 and row['principal_eigenvalue_error']<1e-10 and row['symmetrizer_error']<1e-10 and row['mixing_matrix_minimum_eigenvalue']>-1e-12
    ...
    if complete:
        summary=json.loads((directory/'summary.json').read_text());check('complete count',len(rows)==10 and summary['runs']==rows);base=states['emitter-Y-chi200']
    ...
    output=dict(passed=all(c['passed'] for c in checks),complete=complete,completed_runs=len(rows),declared_runs=10,campaign_passed=campaign_passed,count=len(checks),max_independent_energy_error=max(errors,default=0),checks=checks)
    (ROOT/'emitter-audit.json').write_text(json.dumps(output,indent=2)+'\n',encoding='utf-8')
