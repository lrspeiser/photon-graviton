"""SE-1 six-field Hamiltonian with emitting internal matter oscillators."""
import os
...
class ExchangeEvolution:
    def __init__(self,n=32,length=16.,radius=.9,chi=0.,mix=.5,emission=.4,excitation=.001,angle=0.):
    ...
    def evaluate(self,y,derivatives=True):
        ...
        for i in range(6):
            ...
            X=f[4][sl];xbar=float(np.sum(w*X));difference=Q[i]-self.emission*xbar
