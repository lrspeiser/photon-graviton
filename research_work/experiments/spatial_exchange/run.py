"""SE-1 preregistered controls and eleven source-funded spatial runs."""
import os
...
import time
import numpy as np
from model import ExchangeEvolution,rk4,COMMON
...
def controls():
    ...
    for index in range(48):
        cfg=dict(chi=(-50.,0.,50.,200.)[index%4],mix=(-.5,.5)[(index//4)%2],emission=(0.,.4)[(index//8)%2],n=12,length=10.,radius=1.2)
        ...
        rows.append(dict(index=index,config=cfg,gradient_error=error,minimum_mass=metric['minimum_mass'],cone_error=metric['cone_error'],principal_eigenvalue_error=eigerr,symmetrizer_error=symerr,mixing_matrix_minimum_eigenvalue=mineig,
                         passed=error<2e-6 and metric['minimum_mass']>0 and metric['cone_error']<1e-10 and eigerr<1e-10 and symerr<1e-10 and mineig>-1e-12))
    return dict(rows=rows,passed=all(r['passed'] for r in rows))


def configurations():
    common=dict(n=32,length=16.,radius=.9,dt=.02)
    variants=[dict(name='no-emission',emission=0.,chi=0.,mix=.5),
              dict(name='no-excitation',excitation=0.,chi=200.,mix=.5),
              dict(name='emission-only',chi=0.,mix=0.),
              dict(name='exchange-0',chi=0.,mix=.5),
              dict(name='exchange-50',chi=50.,mix=.5),
              dict(name='exchange-200',chi=200.,mix=.5),
              dict(name='exchange-negative',chi=-50.,mix=.5),
              dict(name='sign-mirror',chi=200.,mix=-.5),
              dict(name='time-refinement',chi=200.,mix=.5,dt=.01),
              dict(name='space-refinement',chi=200.,mix=.5,n=40,dt=.01),
              dict(name='rotation',chi=200.,mix=.5,angle=np.pi/3)]
    return [dict(common,**v) for v in variants]


def main():
    out=ROOT/'evidence-v1';out.mkdir(exist_ok=False)
    manifest=dict(source_commit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),sources={name:hashlib.sha256((ROOT/name).read_bytes()).hexdigest() for name in ('protocol.md','model.py','run.py')},helper_source=dict(path='research_work/experiments/common_cone/model.py',sha256=hashlib.sha256((COMMON/'model.py').read_bytes()).hexdigest()),configurations=configurations(),numpy=np.__version__)
    ...
    by={r['config']['name']:r for r in rows};comparisons=[];base=by['exchange-200']
    base_model=ExchangeEvolution(chi=200.);base_y=np.load(out/'exchange-200.npz')['final'];base_q=base_model.unpack(base_y)[2]
    for name in ('sign-mirror','time-refinement','space-refinement','rotation'):
