> **Archived on 24 September 2026.** This is the project's working notebook through revision 26 (rounds 1–17), with
> every revision note and correction kept. The current write-up is [BLOG.md](../../BLOG.md). Links below point to the
> repository as it was when this was archived.
...
**Rewritten from scratch on 23 September 2026 (rev 12); updated that day and the next (revs 13–26).**
...
* **Rev 26** takes on the one missing link from rev 25: warm matter's pieces fall out of tune, so distant matter can't
  keep step and the pull doesn't grow with the heat. An independent review set the goal (a reason, inside the physics,
  why making more companion doesn't wreck the beat) and proposed a candidate, a matched pair of inner responses (§6.21).
  * **What does the damage, found exactly.** Warm pieces push on each other's beats in two ways: one that always lets a
    group settle, and a sideways one that keeps it churning. Switch off only the sideways half and distant matter keeps
    step at 0.88 with a warm source, better than the 0.63 of a cold one.
  * **The review's pair, reproduced,** number for number. But when both halves shine into the same shared wave, its
    protection is lost. More generally, at equal glow the pushes between warm pieces come out the same size for twelve
    different inner structures: whatever lets a piece send its glow lets it receive its neighbours' just as strongly.
  * **Motion has to enter the way a velocity does.** A velocity flips sign when time runs backwards, and that fixes how
    it can enter the equations; round 16, and the independent calculation before it, had the other form. The right one
    helps: in the full simulation the warm source's wave now pulls distant matter harder as the heat grows (×1.22 of the
    cold pull at a heat weight of 8, where the old form gave ×0.85).
  * **A wave that only travels outward** keeps warm matter in tune. If the companion's crests are carried outward by its
    own stream faster than they can move against it, no piece hears its own echo. In our tests warm sources then keep
    one beat, distant matter keeps step at every distance (0.8–1.0), the pull grows with the heat as our law needs, and
    the glow stays in proportion to mass. It also gives, for round sources, Newton's rule that only the mass inside a
    radius pulls there. Whether the companion's wave really is one-way is the question for the next round.
...
* **the heat term's mechanism (§6.17–6.20):** since rev 24 a first version of the whole chain runs in one experiment,
  ...
  Rev 26 found exactly why (the sideways half of the pushes between warm pieces) and a way out in tests: a companion
  whose wave only travels outward, carried by its own stream. Whether the companion's wave really is one-way is now the
  question. What the store is physically, and the number it sets for the companion's speed, are still to be found;
...
### 6.21 Round 17: keeping warm matter in tune (rev 26)

Round 16 left one link missing: when a source warms up, its pieces fall out of tune with each other, distant matter
can't keep step with its wave, and so the pull doesn't grow with the heat as our law needs. An independent review set
this round's goal in one sentence: find a reason, inside the physics, why making more companion doesn't wreck the beat
that the pull depends on. It proposed three steps, and a candidate: give each piece a matched pair of inner responses
whose pulls on its beat cancel.

**Step A: what exactly knocks a warm piece out of tune.** A piece's loud inner vibrations (its radiators) react a
thousand times faster than its beat drifts, so they can be solved for exactly and taken out. What's left is one equation
for every piece's beat. We checked it two ways: in the full simulation each piece's beat is exactly the sum of three
pushes (to 1 part in 10,000), and the shortened equation gives the same spreads and the same keeping-step as the full
simulation.

The pieces push on each other's beats in two ways. One kind of push is like the pull of a common rhythm: on its own it
always lets a group settle into one beat. The other kind is a sideways push that depends on where the neighbours are in
their own beats, and on its own it can keep a group churning forever. Warmth adds both, and a lot of them: at a heat
weight of 8 the pushes from neighbours' stirred radiators are eight times the cold ones.

Then we switched parts off by hand to see which one does the damage:

| round 16's matter, a heat weight of 8 | distant matter keeping step (1 is perfect) |
|---|---|
| everything as it is | 0.18 |
| **only the "common rhythm" half of the warm pushes kept** | **0.88** |
| only the sideways half kept | 0.00 |
| a cold source, for comparison | 0.63 |

So the culprit is one specific term: the sideways half of the pushes between warm pieces. Remove it, and warmth *helps*:
distant matter keeps step better than with a cold source, because the warm source's wave is stronger.

**Step B: can a piece protect itself?** We rebuilt the review's matched pair and reproduced its numbers: no shift of
the beat at all (4 × 10⁻¹⁹), a 2% shift for a 1% mismatch between the pair, and 80% of the glow kept under collisions.
But in one kind of matter both halves of the pair shine into the same shared wave, and then the surroundings act on the
shared channel instead of on each half separately. The cancellation needs the second and doesn't happen with the first
(the leftover sensitivity is 0.82 of a single radiator's, never zero).

We then tried twelve inner structures, all balancing their energy and all the same in every piece, at equal glow. The
important finding is a limit. **At equal glow, the pushes between warm pieces come out the same size in every
structure** (1.64–1.76 × 10⁻² at a heat weight of 8). That is a basic rule of waves (reciprocity): whatever lets a piece
send its glow into the shared wave lets it receive its neighbours' glow just as strongly. A structure can change the
character of the pushes, not remove them.

Two structures still help, and both keep:
* **Motion has to enter the equations the way a velocity does.** A velocity flips sign when time runs backwards, and a
  push proportional to it must then have a particular mathematical form (the form of the Coriolis force). Round 16, and
  the independent calculation before it, used the other form. With the right one, a whole class of offsets vanishes
  exactly, and distant matter's keeping step at a heat weight of 8 rises from 0.18 to 0.33. This is a correction from
  first principles, not a choice.
* **The review's pair, spread far apart in pitch,** makes each piece a poor echo of its neighbours' waves while it still
  shines when it moves: its own offsets fall tenfold. Together with the velocity's form it is the best structure:
  0.41 at a heat weight of 8 and 0.83 for a cold source (up from 0.63).

And one more lesson: a structure that kept a warm source perfectly together still lost distant matter, because the
whole source settled on a slightly different beat from cold matter. A source's beat has to stay exactly where cold
matter's is, or matter far away, whose hold on the beat weakens with distance, can't follow at all.

**A wave that only travels outward.** All of this points at one thing: the two-way traffic between warm pieces, each
one's wave coming back to it off its neighbours and every pair pushing each other back and forth. What would stop that
is a wave that can't come back. Our companion streams outward from the matter that makes it (§6.20). If its crests are
carried outward by that stream faster than they can move against it, a piece can hear only matter nearer the centre, and
never its own echo.

We tested this directly, by letting the wave carry only outward in the same equations:

| | two-way wave (round 16) | outward-only wave |
|---|---|---|
| warm source (heat weight 8): spread of its pieces' beats | 4.5 × 10⁻³ | 6 × 10⁻⁵ (70 times tighter) |
| distant matter keeping step at 6, 9, 13.5 and 20 wavelengths/2π | 0.19, 0.23, 0.10, 0.09 | **0.98, 0.96, 0.85, 0.81** |
| the pull's growth with heat, measured against what our law needs (1 = exactly right), at those distances | 0.36, 0.79, 0.35, 0.19 | **1.11, 1.21, 1.45, 1.07** |
| a hotter source (heat weight 16), keeping step | 0.12, 0.06, −0.01, 0.06 | **1.00, 0.97, 0.84, 0.77** |

* **With an outward-only wave, warmth no longer spoils the beat.** The innermost matter sets the beat, everything
  farther out locks onto it a quarter beat ahead, and nothing comes back to disturb it. Distant matter keeps step at
  every distance we tried, and better the warmer the source.
* **What matters is that the wave has no way back.** Put the source's pieces in a random one-way order instead, each
  hearing only the ones before it, and it works just as well (keeping step 1.00, 0.99, 0.81, 0.84). The outflow simply
  supplies the natural order: from the centre outward.
* **It has to be nearly one-way.** Letting 10% of the wave travel back inward still keeps most of the effect; 30% or
  more loses it.
* **The glow stays in proportion to mass.** Sources of 24, 48 and 96 pieces give the same intensity per piece, as our
  law needs, not the square of the mass.
* **A bonus.** With a wave that only travels outward, a piece feels only the matter nearer the centre than itself. For a
  round source that is exactly Newton's rule that only the mass inside a radius pulls there, which is the form our law
  already takes.

This is a test of an idea, done by hand in the shortened equations. Making it real means working out the companion as a
flowing medium: how fast its crests move against its own outflow. That is now the clearest next step.

**The same test in the full simulation.** Everything above used the shortened equations. We then imposed the
outward-only wave in the full, energy-balanced simulation of one kind of matter (every piece both sender and receiver,
radiators, stores and forces all included, and whatever the pieces give the wave and its stream booked explicitly),
with 16 receivers on four shells. Two arrangements each; the net pull on a receiver in units of 10⁻⁴:

| source | glow ÷ rest | keeping step | net pull at 6, 9, 13.5 and 20: round 16's form of motion | with motion in a velocity's form |
|---|---|---|---|---|
| at rest | 1 | 0.63 | +1.61, +1.14, +0.41, +0.53 | the same |
| warm, heat weight 2 | 2.0 | 0.78 / 0.87 | +2.17, +1.64, +1.28, +0.54 | +2.01, +1.74, +1.40, +0.99 |
| warm, heat weight 8 | 4.8 | 0.93 / 0.79 | **+2.36, +2.12, +1.75, +1.07** | **+1.50, +1.80, +1.73, +0.88** |
| colliding, heat weight 8 | 1.6 | 0.46 / 0.47 | +1.53, +0.99, +0.38, +0.50 | +1.45, +1.06, +0.37, +0.49 |
| heat weight 8, then brought to rest | 1.0 | 0.62 / 0.70 | +1.85, +1.06, +0.63, +0.11 | +1.67, +1.15, +0.57, +0.76 |

* **For the first time, warm matter pulls distant matter harder than cold matter in the full simulation**: at every
  distance at a heat weight of 2, and at every distance (round 16's form) or three of the four (the velocity's form)
  at a heat weight of 8. With the two-way wave the same matter was pushed at a heat weight of 8.
* **By the review's yardstick** (the extra pull measured against the square root of the wave's extra strength, 1 being
  exactly what our law needs) it comes out between 0.6 and 1.7 at the four distances, about 1 on average (1.06 and
  0.94 at a heat weight of 8, 1.13 and 1.14 at 2), with scatter between the two arrangements and the farthest
  receivers of the cold source still settling.
* **Colliding sources pull like cold ones**, even though they glow a little more: their extra glow flickers too fast to
  be followed. That is our law's rule that colliding gas counts as cold. A source brought to rest goes back to the
  cold pull.
* Energy balances to about 1 part in a trillion (4 in 10 million with collisions).

**Where this leaves us.** The one missing link from round 16 is now understood precisely: the sideways half of the
two-way pushes between warm pieces. No structure inside a piece can remove it, because receiving is the mirror of
sending. But a companion whose wave only travels outward removes it completely, keeps the glow in proportion to mass,
and matches the shape of our law. So the question for the next round is a concrete one about the companion itself: are
its crests carried outward faster than they can move against the stream?

Scripts: `code/rhythm_budget_v17.py`, `code/rhythm_protect_v17.py`, `code/paired_channel_v17.py`, `code/one_way_v17.py`,
`code/one_matter_v17.py`.
...
9. **The theory**, the subject of most of §9.1:
   ...
   * *Done in rev 26 (§6.21):* what puts warm matter out of tune, found exactly: the sideways half of the pushes
     between warm pieces. No structure inside a piece can remove it (receiving mirrors sending), but motion entered as
     a velocity must enter helps, and a companion whose wave only travels outward removes it completely in tests,
     keeping the glow in proportion to mass.
   * *Next:* the companion as a flowing medium: does its outflow carry its crests outward faster than they can move
     against it (one-way), and what does that do to the energy and momentum the pieces exchange with it; then the same
     tests in the full simulation with that wave (distance, heat, collisions, mass, and whether every kind of matter
     falls alike); a physical quiet store with a gap, and u from it; whether a drained receiver can switch its pull off
     (the release factor); the Doppler shifts of real motion; then the guided companion with moving sources, for the
     collisions' memory;
...
* **Revision 25 (round 16).** Rev 24's next steps, one at a time (§6.20):
...
* **Revision 26 (round 17, this page).** Keeping warm matter in tune (§6.21):
  * the damage found exactly: the sideways half of the pushes between warm pieces;
  * an independent review's matched pair reproduced; its protection is lost when both halves shine into one shared
    wave, and at equal glow no inner structure changes the pushes' size;
  * motion entered as a velocity must enter: a correction from first principles that helps;
  * a wave that only travels outward keeps warm matter in tune at every distance tested, with the glow in proportion to
    mass: the lead for the next round.
...
* rev 24's "the exact heat law has been derived" and "the quiet store has to be inside": the mixing was an input, the
  ...
  25, §6.19);
* revs 24–25's motion mixing in the form of a static field (the independent calculation's, and ours in round 16): a
  coupling proportional to a velocity must change sign when time runs backwards, which fixes its form (rev 26, §6.21);
