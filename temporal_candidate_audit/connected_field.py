...
def weights(pos,N):
 # Fixed physical packet width: changing grid resolution does not shrink source.
 width=.02
 ids=np.arange(N);d=(ids/N-pos+.5)%1-.5
 raw=np.exp(-.5*(d/width)**2);w=raw/raw.sum()
 deriv=d/width**2;dw=w*(deriv-w@deriv)
 return ids,w,dw
...
if __name__=='__main__':
 out={'test_ray':[test_field(n) for n in [81,161,321]],'launch_epochs':[test_field(161,t) for t in [10.,20.,40.]],'backreaction':[coupled(n,f) for n in [81,161,321] for f in [.001,.1]],'definition':'Periodic 1D box, smooth fixed-density slabs, positive gradient and quadratic field energies, two fields with epsilon=1e-4; fixed-width Gaussian packet (sigma=0.02 box lengths) with matched interpolation and adjoint deposition. Local particle Hamiltonian Hgamma=p exp(-chi).','limitations':['Synthetic geometry and dimensionless energy ratios, not data calibration.','Photon packet is a 1D ray surrogate, not a complete electromagnetic or gravitational-wave action.','Matter density is fixed; field plus ray energy conserved but matter motion and gravity omitted.','No material clock law or absolute cosmological energy scale is derived.','Pulse-stretch equality tested only in the negligible-backreaction limit.']}
 (O/'connected_field.json').write_text(json.dumps(out,indent=2));print(json.dumps(out,indent=2))
